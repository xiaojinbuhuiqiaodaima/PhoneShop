      ...
      if (results[0]) {
        ...
        res.json({
          ...
          message: {
            ...
            user_order_addr: results[0].user_order_addr || "",
          },
        });
      } else {
        // 新用户
...
router.post("/api/add_order", (req, res) => {
  ...
  console.log("下单接口收到数据：", user_id, address, goods);
  //保存收货地址到用户
  let update_user_addr_sql =
    "UPDATE user_info SET user_order_addr = ?  WHERE id = ?";
  let update_user_addr_params = [address, user_id];
  conn.query(
    update_user_addr_sql,
    update_user_addr_params,
    (error, results, fields) => {
      if (error) {
        res.json({ err_code: 0, message: "更新收获地址失败!" });
        console.log(error);
      }
    }
  );
  ...
  let add_sql =
    "INSERT INTO user_order(goods_content, user_id, address, create_time) VALUES (?, ?, ?, ?)";
  let sql_params = [
    JSON.stringify(goods_content),
    user_id,
    address,
  ...
  conn.query(add_sql, sql_params, (error, results, fields) => {
    if (error) {
      console.log(error);
      res.json({ err_code: 0, message: "下单失败!" });
    } else {
      goods.forEach((item, index, arr) => {
        //减库存
        let get_goods_sql = "SELECT * FROM recommend WHERE goods_id = ?";
        let get_goods_sql_params = [item.goods_id];
        conn.query(
          get_goods_sql,
          get_goods_sql_params,
          (error, results, fields) => {
            if (error) {
              res.json({ err_code: 0, message: "查询库存失败!" });
              console.log(error);
            } else {
              if (!results) {
                console.log("查询结果为空，不进行库存更新 ", results);
              } else {
                results = JSON.parse(JSON.stringify(results));
                let update_goods_counts_sql =
                  "UPDATE recommend SET counts = ?  WHERE goods_id = ?";
                let now_counts = results[0].counts - item.counts;
                let update_goods_sql_params = [now_counts, item.goods_id];
                conn.query(
                  update_goods_counts_sql,
                  update_goods_sql_params,
                  (error, results, fields) => {
                    if (error) {
                      res.json({ err_code: 0, message: "更新库存失败!" });
                      console.log(error);
                    } else {
                      console.log("更新库存成功");
                    }
            ...
            //清空购物车
            let del_cart_sql =
              "DELETE FROM cart WHERE goods_id = ? and user_id = ?";
            let del_cart_sql_params = [item.goods_id, user_id];
            conn.query(
              del_cart_sql,
              del_cart_sql_params,
              (error, results, fields) => {
                if (error) {
                  res.json({ err_code: 0, message: "清空购物车失败!" });
                  console.log(error);
                } else {
                  console.log("清空购物车成功");
                }
 ...
 * 获取所有该用户的订单
 */
router.post("/api/get/order", (req, res) => {
  // 获取数据
  const user_id = req.body.user_id;
  console.log("获取所有该用户的订单接口收到数据：", user_id);
  let get_orders_sql =
    "SELECT * FROM user_order WHERE user_id = ? ORDER BY create_time DESC";
  let get_orders_sql_params = [user_id];
  conn.query(
    get_orders_sql,
    get_orders_sql_params,
    (error, results, fields) => {
      if (error) {
        res.json({ err_code: 0, message: "查询订单失败!" });
        console.log(error);
      } else {
        results = JSON.parse(JSON.stringify(results));
        const respRes = [];
        results.forEach((item, index, arr) => {
          respRes.push({
            id: item.order_id,
            goods: JSON.parse(item.goods_content),
            status: item.status,
            address: item.address,
            create_time: item.create_time,
          });
        });
        res.json({ success_code: 200, message: respRes });
      }
    }
  );
});
 ...
 * 获取所有的订单
 */
router.post("/api/get/admin_order", (req, res) => {
  let get_orders_sql = "SELECT * FROM user_order ORDER BY create_time DESC";
  conn.query(get_orders_sql, (error, results, fields) => {
    if (error) {
      res.json({ err_code: 0, message: "查询订单失败!" });
      console.log(error);
    } else {
